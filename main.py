...
def get_chat_completion(prompt):
    api_key = os.getenv("GROQ_API_KEY")  # Use the API key from environment variables
...
def process_question(question: str, file_path: Optional[str] = None) -> str:
    try:
        ...
        # Check if the content is a string, return it directly
        if isinstance(content, str):
            return content

        # If the content is structured, process tool calls
        if isinstance(content, list):
            logger.info(f"Tool calls detected: {len(content)}")
            for tool_call in content:
                try:
                    ...
                    # Parse function arguments if they are in JSON format
                    if isinstance(function_args, str):
                        try:
                            function_args = json.loads(function_args)
                        except json.JSONDecodeError:
                            logger.error(f"Invalid JSON in tool arguments: {function_args}")
                            function_args = {}
